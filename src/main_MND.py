...
from Model_deForMND_lstm import *
from utils import *
import torch
...
# test_features_path = '../features/timit_test_features.json'
# test_targets_path = 'F:/datasets/timit_test_phonemes.txt'
features_path_test = '../features/timit_train_features_demo.json'
targets_path_test = 'F:/datasets/timit_train_phonemes_demo.txt'

max_length = 18 # control the maximum translation length.
num_epochs = 50 # number of times training the whole train dataset
batch_size = 100 # number of words/frames trained in each input
vocab_size = 5500
device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
# shape of features: (4620, 371, 304)
# shape of gt.txt: (4620, 18, 1)
...
def evaluate(model, asrdata):
    ...
    for i, data in enumerate(iterator_test):
        ...
        # softmax_cal=(M*N,dict);target_cal=(M*N);outputs=(N,M,1)
        loss = criterion(softmax_cal, target_cal)
        epoch_loss += loss
        print("Test Loss at batch %d: %.2f" % (i, loss))
        # Evaluation Metrics prepare
        for j in range(target.shape[0]):
            # target.shape = (batch_size,len(sentens),1), new target[i].shape =(18, 1)-->(18, 1, 1)
            target_clean = []
            outputs_clean = []
            for n in range(target[j].shape[0]): ## target[j] is a sentence # target[j][n] is a number, refer to the word id
                if target[j][n] != padINDEX and target[j][n] != sosINDEX and target[j][n] != eosINDEX and target[j][n] != unkINDEX:
                    target_clean.append([target[j][n].item()])
                    outputs_clean.append(outputs[j][n])
            truth = asrdata.vec_to_sentence(np.expand_dims(target_clean, axis = 2))
...
def train(model, asrdata):
    ...
    for epoch in range(num_epochs):
        ...
        for i, data in enumerate(iterator_train):
            input, target = data
            optimizer.zero_grad()
            # softmax_cal, target_cal, outputs = model(input.float(), target.long()); #batch_first = false
            softmax_cal, target_cal, outputs = model(input.float().transpose(0,1).to(device),
                                                     target.long().transpose(0,1).to(device),
            ...
            if i % 100 == 0: ##print one step target and preditions to see the result
                for j in range(target.shape[0]):
                    # target.shape = (batch_size,len(sentens),1), new target[i].shape =(18, 1)-->(18, 1, 1)
                    print('The sentence', j, ' is:')
                    print("Truth: \"%s\"" % asrdata.vec_to_sentence(np.expand_dims(target[j].cpu().detach(), axis=2)))
