...
from DataLoader_initialpad_ctc import *
from Model_deForMND_bigru_att_ctc import *
from utils import *
import torch
...
features_path_test = '../features/timit_whole_test_mfcc_energy_gradient1_585_26.json'
targets_path_test = '../targets/timit_whole_test_phonemes_noh.txt'

num_epochs = 3 # number of times training the whole train dataset
# batch_size = 64 # for sentenses
batch_size = 10 # for phonemes 30
initial_lr = 0.001
ctcRate = 0.5  #best performance is 0.5
...
def train(model, asrdata):
    ...
    # #part no need to smaller learning rate yet!
    scheduler_1 = MultiStepLR(optimizer_1, milestones=[150, 250], gamma=0.1)
    ...
    for epoch in range(num_epochs):
        ...
        torch.save(model.state_dict(), "./models/TestingTesting_per_bigru_att_ctc.5_weights_trainComG1_E3_B10_testComCor_wer.._bleu...pth")
...
def main():
    ...
    # part change the model for only evaluation
    model.load_state_dict(torch.load("./models/TestingTesting_per_bigru_att_ctc.5_weights_trainComG1_E3_B10_testComCor_wer.._bleu...pth"))
