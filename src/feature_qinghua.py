...
def main():
    ...
    samePathTxt = open(os.path.join('../datasets', inputDataset + '_' + dataset_usage + '_samePath.txt'), 'r')
    Lines = samePathTxt.readlines()
    for line in Lines:
        wav_filesPath = line+'.wav'
    ...
    for filename in filenames:
        ...
        #提取所有特征
        wav_features = getFeature(filename) #get wav_features.shape == ( frames,(39+39+13+13+200) )=(frames, 304)
        #提取其中单个或者组合特征
        # demof_fbank1 = fbank_1(filename) #fbank1() 返回 帧数*39 的fbanks参数
        # demof_mfcc1_fbank1 = np.hstack((fbank_1(filename)))  # fbank1() 返回 帧数*39 的fbanks参数
        # wav_features = demof_mfcc1_fbank1
    ...
    threshood = math.ceil(len(filenames)* 0.8)  #80%个数据是指第多少条语音
    frameSize = int(wavs_framesNum_sorted[threshood]) #排序后，第80%个数据有多少帧
    print('80%frameSize:', frameSize)

    frameSize = 371 #change the frameSize as same as the frameSize in the timit train set
...
#### pad for the frames in each sentence
    ...
    json_file_Path = os.path.join('../features', inputDataset + '_' + dataset_usage + '_mfcc_lpcc.json')
    store(json_data,json_file_Path)
...
# 伪代码
        #total_features = []
        #for wav_data in dataset:
        #   wav_feature_combined = feature_extract(wav_data)
        #   total_features.append(wav_feature_combined)
        #   total_feature = np.array(total_feature)
        #   print(total_features.shape)  #shape = (语音数据个数*(frameNum*sum(feature1to6)))
