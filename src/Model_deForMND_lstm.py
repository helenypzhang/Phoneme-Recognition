...
class DecoderLSTM(nn.Module):
    def __init__(self, dict_size, embedding_size=52, hidden_size=100):
    ...
    def forward(self, input, hidden_state, cell_state):
        ...
        output, (hidden_state, cell_state) = self.lstm(embedded.float(), (hidden_state.float(), cell_state.float()))
...
class AsrModel(nn.Module):
    ...
    def forward(self, input, target, teacher_forcing_ratio: float = 0.5): #input.shape=(N,frames,F), target.shape=(N,M,1)
        ...
        for iphn in range(max_phn_num):
            decoder_output, decoder_softmax, decoder_hidden, decoder_cell = self.decoder.forward(decoder_input, decoder_hidden, decoder_cell)
