...
class Conv2dSubampling(nn.Module):
    ...
    def forward(self, inputs, input_lengths):
        print('conv inputs shape:', inputs.shape)
        outputs = self.sequential(inputs.unsqueeze(1))
        print('conv outputs shape:', outputs.shape)
        batch_size, channels, subsampled_lengths, sumsampled_dim = outputs.size()

        outputs = outputs.permute(0, 2, 1, 3)
        outputs = outputs.contiguous().view(batch_size, subsampled_lengths, channels * sumsampled_dim)
        print('conv final outputs shape:', outputs.shape)
...
class EncoderGRU(nn.Module):
    def __init__(self, dict_size, embedding_size=96, encoder_hidden_size=100, decoder_hidden_size = 100):
        super(EncoderGRU, self).__init__()
        self.conv_subsample = Conv2dSubampling(in_channels=1, out_channels=2)
    ...
    def forward(self, input):
        ...
        '''
        embedded = self.dropout(input) #do not change shape
        ...
        '''
        conv_inputs = embedded.transpose(0,1)
        conv_input_lengths = embedded.shape[1]
        conv_outputs, conv_output_lengths = self.conv_subsample(conv_inputs, conv_input_lengths)
        conv_outputs = conv_outputs.transpose(0,1)
        # print('conv_outputs.shape:', conv_outputs.shape)

        outputs, hidden_state = self.gru(conv_outputs.float())
        ...
        return ctc_outputs, outputs, hidden_state
...
class DecoderGRU(nn.Module):
    def __init__(self, dict_size, embedding_size=96, encoder_hidden_size=100, decoder_hidden_size=100):
        ...
        self.embedding = nn.Embedding(dict_size, embedding_size) #(original) --> (originial,embedding_size); add one dim
...
class AsrModel(nn.Module):
    def __init__(self, dict_size, max_length, sos, pad):
        ...
        self.encoder = EncoderGRU(self.dict_size)
    ...
    def forward(self, input, target, teacher_forcing_ratio: float = 0.5): #input.shape=(N,frames,F), target.shape=(N,M,1)
        ...
        # encoder_ctc_outputs = torch.argmax(encoder_ctc_outputs,2)[:, :, np.newaxis] #[F,N,D]-->[F,N,1]
